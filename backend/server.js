...
const brevoFromEmail = envFromEmail || 'salim@kns.sl';
const brevoToEmail =
...
app.get('/api/scholarships/:id/download/:type', async (req, res) => {
    ...
    try {
        ...
        const fullPath = resolveScholarshipFilePath(path.join(__dirname, '..'), trimmedPath);
        if (!fullPath) {
...
app.post('/api/scholarship-applications', formRateLimiter, upload.none(), async (req, res) => {
    try {
        // pull fields off the request body (multer processes multipart/form-data)
        const {
            scholarship_id,
            surname,
            first_name,
        ...
        if (brevoApiKey && brevoFromEmail && brevoToEmail) {
            // Get scholarship title for email subject
            let scholarshipTitle = programme;
            if (scholarship_id) {
                try {
                    const scholarship = await db.getScholarshipById(scholarship_id);
                    if (scholarship && scholarship.title) {
                        scholarshipTitle = scholarship.title;
                    }
                } catch (error) {
                    console.error('Error fetching scholarship title for email:', error);
                }
            }

            const subjectLine = `${scholarshipTitle}`;
...
`.trim();
            ...
            const htmlBody = `
                ...
                <p>${escapeHtmlWithBreaks(personal_statement.substring(0, 500))}${personal_statement.length > 500 ? '...' : ''}</p>
