...
function ensureTrainingsNavLink() {
    ...
    const programmesLink = Array.from(navMenu.querySelectorAll('a')).find(function(link) {
        const href = (link.getAttribute('href') || '').toLowerCase();
        return href.indexOf('programmes.html') !== -1;
    });
    if (programmesLink && programmesLink.parentElement) {
        programmesLink.parentElement.insertAdjacentElement('afterend', trainingsItem);
        return;
    }
